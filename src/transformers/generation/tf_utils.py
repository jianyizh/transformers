...
class TFGenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        use_xla = False #not tf.executing_eagerly()
    ...
    def _update_model_kwargs_for_generation(
        self, outputs: ModelOutput, model_kwargs: Dict[str, Any], is_encoder_decoder: bool = False, batch_size: int = 1, beams: int = 1
    ) -> Dict[str, Any]:
    ...
    def _update_model_kwargs_for_xla_generation(
        ...
        def _update_attention(model_kwargs, new_past_index, is_encoder_decoder):
            ...
            if is_encoder_decoder:
                ...
                attention_mask = model_kwargs.pop("attention_mask")
                attention_mask_update_slice = tf.ones((shape_list(attention_mask)[0], 1), dtype=attention_mask.dtype)
        ...
        if not is_past_initialized:
            ...
            if shape_list(new_past[0][0])[0] > shape_list(mask["attention_mask"])[0] and shape_list(mask["attention_mask"])[0]>1:
                    mask["attention_mask"] = tf.tile(mask["attention_mask"],[shape_list(new_past[0][0])[0]//shape_list(mask["attention_mask"])[0],1])
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        all_shapes = (
                            generated.get_shape(),
                            finished_sequences.get_shape(),
                            tf.TensorShape([]),
                            dict({'attention_mask':tf.TensorShape([batch_size,None]),
                                  'past_key_values':tuple(
                                      (tf.TensorShape([batch_size,16,None,256]), tf.TensorShape([batch_size,16,None,256]))
                                      for _ in range(28)),
                            })
                        )
        ...
        **model_kwargs,
        ...
        with tf.control_dependencies([a]):
            ...
            def beam_search_body_fn(
                ...
                is_first = False
                print("111111")
                if past_key_values is None or needs_full_input:
                ...
                logits = unflatten_beam_dim(model_outputs.logits[:, -1], 1 if (is_first) else num_beams)
                # 2. Compute log probs
                ...
                log_probs = unflatten_beam_dim(log_probs, 1 if (is_first) else num_beams)
                ...
                if "past_key_values" in model_outputs:
                    cache = tf.nest.map_structure(
                        lambda tensor: unflatten_beam_dim(tensor, 1 if is_first else num_beams, batch_axis=cache_batch_axis),
                ...
                with tf.control_dependencies([b]):
                    ...
                    def gen2():
                        ...
                        all_shapes = (
                            tt.get_shape(),
                            cur_len.get_shape(),
                            running_sequences.get_shape(),
                            running_scores.get_shape(),
                            running_beam_indices.get_shape(),
                            sequences.get_shape(),
                            scores.get_shape(),
